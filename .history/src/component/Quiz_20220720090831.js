                          ...
                          {scoreLoading ? (
                            ...
                            >
                              ...
                              <div  style={{ width: 200, height: 200 }}>
                                <CircularProgressbar className="percentage" text={`${item.rply}%`} value={parseInt(item.rply)}
                                 styles={buildStyles({
                                  // Text size
                                  textSize: '16px',
                                  // Colors
                                  pathColor: item.scoreType === "high" ? `#008450` : item.scoreType === "mild" ? '#EFB700' : 'rgb(208, 42, 42)',
                                  textColor: item.scoreType === "high" ? `#008450` : item.scoreType === "mild" ? '#EFB700' : 'rgb(208, 42, 42)',
                                  trailColor: 'gray',
                              ...
                              <div className="finalskills">
                                ...
                                {chatArr.map((item, index) => (
                                    <>
                                      {item.id === "painscale" && (
                                        <>
                                          Complaint in {part} with Pain : <img src={item.answer[0]} width={50} height={50}/>
                                        </>
                                      )}
                                    </>
                                  ))}
                                    ...
                                    <>
                                      {item.section === "PostureFlex" && (
                                        <>
                                          {item.answer[0] !== "No" && (
                                            <>
                                              {item.answer[1].label.toLowerCase() === part.toLowerCase() && item.answer[1].angle}
                                              b
                                              <img
                                                src={`${item.answer[0]}`}
                                                style={{
                                    ...
                                    onClick={() => {
                                      if (Array.isArray(option)) {
                                        if (option[0] === "Yes") {
                                          ...
                                          if (
                                            window.confirm("AROM & Posture Check enable real time assessment of joint flexibility and lifestyle induced postural problems. Privacy is ensured as no video is recorded and only joint data is stored. Are you sure you would not like to go ahead with an in depth analysis of your problem?") ===
                                            true
                                          ) {
                                            computeAns(option, crrqst);
                                          }
