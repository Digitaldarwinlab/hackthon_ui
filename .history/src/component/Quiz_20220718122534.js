...
const Quiz = () => {
  ...
  const computeAns = async (ans, qst) => {
    ...
    if (crrqst.isInput) {
      ...
      let temp = {
        ...qst,
        answer: qst.section === "AromFlex"  ? qst.ans[0] !== 'No' We'll Like to help you with it : ans,
        rply: randomWords[Math.floor(Math.random() * 5)],
      };
                      ...
                      {item.type === "rpt" && (
                        <div className="card">
                          <div className="card-details">
                            ...
                            {item.login === true &&(
                            <a style={{textDecoration:'none'}} href='/login' target="_blank" rel="noopener noreferrer"/>
                            )}
                          </div>
                        </div>
                      )}
