    ...
    if (check) {
      ...
      if (check.length !== JSON.parse(localStorage.getItem("qst")).length) {
        ...
        if (a !== -1) {
          ...
          setLoading(false);
          if(JSON.parse(localStorage.getItem("qst"))[a + 1].posture_type ){setCrrposterType(JSON.parse(localStorage.getItem("qst"))[a + 1].posture_type)}
        }
        // setTempText("dummy");
      } else {
        setLoading(false);
      ...
      if(JSON.parse(localStorage.getItem("qst"))[0].posture_type ){setCrrposterType(JSON.parse(localStorage.getItem("qst"))[0].posture_type)}
    }
  }
  ...
  const sendOtp = async () => {
    try {
      ...
      if (responseData === [] || responseData.status_code === 300) {
        ...
        if(resp[0].posture_type ){setCrrposterType(resp[0].posture_type)}
  ...
  const computeAns = async (ans, qst) => {
    ...
    if (crrqst.isInput) {
      ...
      setTimeout(() => {
        ...
        if(JSON.parse(localStorage.getItem("qst"))[ind + 1].posture_type ){setCrrposterType(JSON.parse(localStorage.getItem("qst"))[ind + 1].posture_type)}
        setCrransoptimg(
