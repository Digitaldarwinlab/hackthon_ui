  ...
  const computeAns = async (ans, qst,flag,index) => {
    ...
    if (crrqst.isInput) {
      ...
      setTimeout(() => {
        setLoading(false);
        //setChatArr([...chatArr, temp]);
        console.log(ind !== JSON.parse(localStorage.getItem("qst")).length - 1)
        if (ind !== JSON.parse(localStorage.getItem("qst")).length - 1) {
          setCrrQst(replaceText(qst, ans,flag,index));
          if (flag){
            let a = parseInt(localStorage.getItem('AromFlexLength')) - index
