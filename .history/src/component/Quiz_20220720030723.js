  ...
  const computeAns = async (ans, qst) => {
    ...
    if (crrqst.isInput) {
      ...
      if (
        ...
        temp.type = "score";
        let rpl = await parseInt(a.score).toFixed() < 40 ? 'Mild' : 
        temp.scoreRply =`Dear Sushant, Based on the information you have shared and my analysis; your "General" score for "Knee" is at ${parseInt(a.score).toFixed()}%. This puts you at a "Mild/ Medium/ High" risk. To help you manage it, please help us with a few more detail`
        setChatArr([...chatArr, temp]);
        localStorage.setItem("chat", JSON.stringify([...chatArr, temp]));
        setTimeout(async () => {
