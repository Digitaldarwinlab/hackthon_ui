  ...
  const computeAns = async (ans, qst, flag, index) => {
    ...
    if (crrqst.isInput) {
      ...
      if (condition) {
        ...
        let scor = 100 - parseInt(a.score).toFixed()) < 40
        if ((100 - parseInt(a.score).toFixed()) < 40) {
          temp.scoreType = "low";
        } else if ((100 - parseInt(a.score).toFixed()) < 70) {
          temp.scoreType = "mild";
        } else if ((100 - parseInt(a.score).toFixed()) > 70) {
          temp.scoreType = "high";
        }
        temp.rply = `${100 - parseInt(a.score).toFixed()}`;
        setGeneralScore(100 - parseInt(a.score).toFixed());
        localStorage.setItem("generalScore", 100 - parseInt(a.score).toFixed());
        temp.type = "score";
        let a1 = [];
        a1.push(
          `Dear ${firstname}, Based on the information you have shared and my analysis, your General score for ${part} is at ${parseInt(
            a.score
          ).toFixed()}%(Higher scores indicate healthier condition).`
        );
        a1.push(
          `This puts you at a ${
            s < 40
