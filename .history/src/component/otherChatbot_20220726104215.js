...
const ChatBot = () => {
    ...
    if (crrqst.isInput) {
      ...
      if (!localStorage.getItem("userId"))  {
        if (JSON.parse(localStorage.getItem("qst")).length === 1) {
          ...
          a.push(
            `Dear ${firstname}, thank you for initiating an assessment. I understand that you spend ${time} doing ${activity} activity and this leads to ${part} pain. Your BMI was assessed as ${bmiScore} and puts you in ${bmiScore<18.5 ? 'Underweight' : bmiScore > 18.5 && bmiScore < 24.9 ? 'Normal' : bmiScore > 24.9 && bmiScore < 29.9 ? 'Overweight' : bmiScore >29.9 && bmiScore < 34.9 ? 'Obese' : 'Extremely Obese' }`
          );
          a.push(
      ...
      let finalCondition =
        localStorage.getItem("index") !== null
          ...
          : parseInt(localStorage.getItem("demographicLength")) +
            ...
            parseInt(localStorage.getItem("ConsentLength"));
      setLoading(true);
      if (localStorage.getItem("userId") && JSON.parse(localStorage.getItem("qst")).length === 1 ) {
        setLoading(false);
        setRptLoading(true);
        temp.rply ="Thank you for initiating an assesssment. We'll Like to help you with this and for muscle strengthening & conditioning to get a better understanding of your condition and design a personalized therapy schedule."          
        temp.type = "rpt";
        setChatArr([...chatArr, temp]);
  ...
  const getBmi = (crrqst) => {
    let bmi;
      ...
      await localStorage.setItem("bmi", bmi.toFixed(2));
    bmi2();
