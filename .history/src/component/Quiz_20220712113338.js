...
const Quiz = () => {
    ...
    let responseData = [
      {
        ...
        option: [["Done",1],["Done",1],["Done",1]],
        pp_qs_id: 5,
        type: "qst",
        isInput: true,
        id: "emoji",
        emoji_image:[
          'https://dev.physioai.care/images/t1/pain_1.png',
          'https://dev.physioai.care/images/t1/pain_1.png',
          'https://dev.physioai.care/images/t1/pain_1.png',
        ]
      },
    ...
    if (check) {
      ...
      if (check.length !== JSON.parse(localStorage.getItem("qst")).length) {
        ...
        if (a !== -1) {
          ...
          setCrrAnsemoji(JSON.parse(localStorage.getItem("qst"))[a + 1].emoji_image)
          setLoading(false);
        }
        // setTempText("dummy");
      } else {
  ...
  const computeAns = async (ans, qst) => {
    ...
    if (crrqst.isInput) {
      ...
      setTimeout(() => {
        setLoading(false);
        setCrrQst(replaceText(qst, tempText ? tempText : firstname + ' ' + lastname));
        setCrrAns(JSON.parse(localStorage.getItem("qst"))[ind + 1].option);
        setCrrAnsemoji(JSON.parse(localStorage.getItem("qst"))[ind + 1].emoji_image)
      }, 3000);
    } else {
      let temp = {
        ...qst,
                              ...
                              <>
                                {crrans.map((option) => (
                                  <button
                                    onClick={() => {
                                      //    console.log("chat ")
                                      if (tempText.length > 0 || (firstname.length>0 && lastname.length>0)) {
