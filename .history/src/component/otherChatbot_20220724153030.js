...
const ChatBot = () => {
  ...
  const [postureDone, setPostureDone] = useState(JSON.parse(localStorage.getItem('postureDone')));
  ...
  const computeAns = async (ans, qst,flag,index) => {
    ...
    if (crrqst.isInput) {
      ...
      if (condition) {
        ...
        JSON.parse(localStorage.getItem("chat")).length) && !flag 
      ) {
        setLoading(false);
      ...
      setTimeout(() => {
        ...
        if (ind !== JSON.parse(localStorage.getItem("qst")).length - 1) {
          setCrrQst(replaceText(qst, ans,flag,parseInt(localStorage.getItem('AromFlexLength')) -index + 1 ));
          if (flag){
            localStorage.setItem('index',parseInt(localStorage.getItem('AromFlexLength')) - index + 1)
            let a = parseInt(localStorage.getItem('AromFlexLength')) - index + 1
            console.log(a,ind)
    ...
    <>
      {startAssesment === false ? (
        ...
        <>
          ...
          parseInt(localStorage.getItem("GeneralLength")) +
          parseInt(localStorage.getItem("PainScaleLength")) +
          parseInt(localStorage.getItem("PostureFlexLength")) + 1 ===
        JSON.parse(localStorage.getItem("chat")).length ? (<>{crrans.map((option,index) => (
          <button
            onClick={() => {
              console.log(index)
              if (Array.isArray(option)) {
