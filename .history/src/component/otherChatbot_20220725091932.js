...
const ChatBot = () => {
  ...
  const computeAns = async (ans, qst, flag, index) => {
    ...
    if (crrqst.isInput) {
      ...
      setChatArr([...chatArr, temp]);
      localStorage.setItem("chat", JSON.stringify([...chatArr, temp]));
      let finalCondition = localStorage.getItem("index") ? JSON.parse(localStorage.getItem("chat")).length - parseInt(localStorage.getItem("AroLength")) - 1 :  JSON.parse(localStorage.getItem("chat")).length
      setLoading(true);
