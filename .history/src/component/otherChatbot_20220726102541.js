    ...
    if (crrqst.isInput) {
      ...
      if (localStorage.getItem("userId")) {
        ...
        if (JSON.parse(localStorage.getItem("qst")).length === 1) {
          ...
          a.push(
            `Dear ${firstname}, thank you for initiating an assessment. I understand that you spend ${time} doing ${activity} activity and this leads to ${part} pain. Your BMI was assessed as ${bmiScore} and puts you in ${bmiScore<18.5 ? 'Underweight' : bmiScore > 18.5 && bmiScore < 24.9 ? 'Normal' : bmiScore > 24.9 && bmiScore < 29.9 ? ''}`
          );
          a.push(
